...
class PromptHandler:
    ...
    def handle(self) -> None:
        ...
        try:
            ...
            print(f"[error] Exception occurred in handle_prompt: {type(e).__name__}: {e}")
            traceback.print_exc()
            ...
            if hasattr(self.args, 'verbose') and self.args.verbose:
                ...
                if token_info:
                    if isinstance(token_info, dict):
                        token_str = " | ".join(f"{k}: {format_tokens(v) if isinstance(v, int) else v}" for k, v in token_info.items())
                    else:
    ...
    def _print_exit_reason_and_parts(self, final_event):
        exit_reason = getattr(final_event, 'metadata', {}).get('exit_reason') if hasattr(final_event, 'metadata') else None
        if exit_reason:
        ...
        if not exit_reason:
            if parts is None or len(parts) == 0:
                ...
                if hasattr(self.args, 'verbose_agent') and self.args.verbose_agent:
                    print("[yellow]No user-visible output. Model returned the following parts:")
                    for idx, part in enumerate(parts):
                        print(f"  [part {idx}] type: {type(part).__name__} | content: {getattr(part, 'content', repr(part))}")
