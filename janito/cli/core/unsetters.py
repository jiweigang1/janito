...
def handle_unset(args):
    ...
    if '.' in key:
        ...
        if len(parts) == 2:
            ...
            current_val = global_config.file_config.get('providers', {}).get(provider, {}).get(subkey)
            ...
            model_conf = global_config.file_config.get('providers', {}).get(provider, {}).get('models', {}).get(model, {})
            current_val = model_conf.get(subkey)
            if current_val is not None:
                del global_config.file_config['providers'][provider]['models'][model][subkey]
                global_config.save()
                print(f"{key}={current_val} was removed.")
                return True
    else:
