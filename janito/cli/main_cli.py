...
definition = [
    ...
    (["--verbose-tools"], {"action": "store_true", "help": "Print info messages for tool execution in tools adapter."}),
    (["--verbose-agent"], {"action": "store_true", "help": "Print info messages for agent event and message part handling."}),
...
MODIFIER_KEYS = ["provider", "model", "role", "system", "temperature", "verbose", "raw", "no_termweb", "termweb_port", "verbose_api", "verbose_tools"]
...
class JanitoCLI:
    ...
    def run(self):
        ...
        if run_mode == RunMode.GET and (self.args.list_providers
        ...
        if run_mode == RunMode.RUN:
            ...
            handle_runner(self.args, provider, llm_driver_config, agent_role, verbose_tools=self.args.verbose_tools)
