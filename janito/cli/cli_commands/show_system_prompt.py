...
def handle_show_system_prompt(args):
    ...
    modifiers = {k: getattr(args, k) for k in MODIFIER_KEYS if getattr(args, k, None) is not None}
    ...
    templates_dir = Path(__file__).parent.parent.parent / "agent" / "templates" / "profiles"
    ...
    if template_path.exists():
        ...
        try:
            ...
            print(f"[janito] Could not find system_prompt_template_main.txt.j2 in {template_path} nor in janito.agent.templates.profiles package.")
